...
export default function tickersReducer (state = initialState, action:ActionsType): ITicker {
  switch (action.type) {
    case 'TickersList/GET_LIST':
      return {...state, tickersList: action.payload}
    case 'TickersList/GET_DATA':
      return {...state, tickersData: action.payload.quotes, fetchingInterval: action.payload.interval}
    case 'TickersList/FILTER_TICKERS':
      return {...state, tickersData: [...state.tickersData.filter(el=>el.ticker !== action.payload)]}
    case 'TickersList/ADD_TICKER':
